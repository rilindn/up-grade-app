...
export default {
  ...
  methods: {
    ...
    async handleSubmit() {
      ...
      try {
        const result = await updateSubjectTeacher(id, data);
        if (result?.status === 200) {
          ...
          this.$notify({
            type: "success",
            duration: 2000,
            text: "Role assignment was updated successfully!",
          });
        } else {
          this.loading = false;
          this.$notify({
